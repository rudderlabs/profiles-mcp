...
class ProfilesTools:
    ...
    FAKE_NAME_PATTERNS = {
        ...
        "user_confirmed", "user_chosen"  # Even these placeholders should be replaced
    }
    ...
    def _analyze_project_structure(self, project_path: str) -> dict:
        ...
        if not os.path.exists(project_abs_path):
            analysis["errors"].append(f"Project directory does not exist: {project_abs_path}")
            return analysis
        ...
        try:
            ...
            if not os.path.exists(pb_project_path):
                analysis["warnings"].append("pb_project.yaml not found. This might be a new/greenfield project.")
            ...
            try:
                with open(pb_project_path, 'r', encoding='utf-8') as f:
                    ...
                    model_folders = pb_config.get("model_folders") if pb_config else None

                    if model_folders is None:
                        analysis["errors"].append("pb_project.yaml is missing required 'model_folders' key.")
                        return analysis

                    if not isinstance(model_folders, list):
                        analysis["errors"].append("'model_folders' in pb_project.yaml must be a list.")
                        return analysis

                    if not model_folders:
                        analysis["errors"].append("'model_folders' in pb_project.yaml cannot be empty.")
                        return analysis

                    analysis["model_folders"] = model_folders

            except yaml.YAMLError as e:
                analysis["errors"].append(f"Failed to parse pb_project.yaml - invalid YAML syntax: {str(e)}")
                return analysis
            except PermissionError:
                analysis["errors"].append(f"Permission denied reading pb_project.yaml: {pb_project_path}")
                return analysis
            except FileNotFoundError:
                analysis["errors"].append(f"pb_project.yaml not found at expected path: {pb_project_path}")
                return analysis
            except Exception as e:
                analysis["errors"].append(f"Unexpected error reading pb_project.yaml: {str(e)}")
                return analysis
            ...
            for model_folder in analysis["model_folders"]:
                ...
                if not os.path.exists(folder_path):
                    analysis["warnings"].append(f"Model folder does not exist: {model_folder}")
                    continue

                if not os.path.isdir(folder_path):
                    analysis["warnings"].append(f"Model folder is not a directory: {model_folder}")
                    continue
                ...
                for pattern in yaml_patterns:
                    folder_yamls = glob.glob(os.path.join(folder_path, "**", pattern), recursive=True)
            ...
            if analysis["summary"]["model_folders_found"] != analysis["summary"]["model_folders_scanned"]:
                analysis["warnings"].append("Some model folders specified in pb_project.yaml were not found on disk.")
...
"""
    ...
    def workflow_guide(self, user_goal: str, current_action: str = "start", user_confirmed_tables: str = "", user_confirmed_connection: str = "", knowledge_phase_completed: str = "") -> dict:
        ...
        **MANDATORY FIRST TOOL**: Your complete workflow guide for profiles projects.
        ...
        guide = self._initialize_guide_structure(user_goal, current_action, current_year)

        # Validate knowledge gathering phase completion for configuration actions
        config_validation = self._validate_knowledge_phase(current_action, knowledge_phase_completed)
        if not config_validation["valid"]:
            return self._merge_validation_results(guide, config_validation)

        # Validate user confirmations
        user_validation = self._validate_user_confirmations(current_action, user_confirmed_tables, user_confirmed_connection)
        ...
        if handler:
            return handler(guide, user_goal, user_confirmed_tables, user_confirmed_connection, knowledge_phase_completed)
        else:
            return self._handle_unknown_action(guide, current_action)

    def _initialize_guide_structure(self, user_goal: str, current_action: str, current_year: int) -> dict:
    ...
    def _handle_start_action(self, guide: dict, user_goal: str, user_confirmed_tables: str, user_confirmed_connection: str, knowledge_phase_completed: str) -> dict:
    ...
    def _handle_knowledge_gathering_action(self, guide: dict, user_goal: str, user_confirmed_tables: str, user_confirmed_connection: str, knowledge_phase_completed: str) -> dict:
    ...
    def _handle_discover_resources_action(self, guide: dict, user_goal: str, user_confirmed_tables: str, user_confirmed_connection: str, knowledge_phase_completed: str) -> dict:
        """Handle the 'discover_resources' action."""
        task_lower = user_goal.lower()

        if any(word in task_lower for word in ["new", "setup", "initialize", "create project"]):
            guide["next_tools"] = ["setup_new_profiles_project", "get_existing_connections", "input_table_suggestions"]
            guide["workflow_steps"] = [
            ...
            guide["next_tools"] = ["get_existing_connections", "input_table_suggestions", "describe_table"]
            guide["workflow_steps"] = [
    ...
    def _handle_create_inputs_yaml_action(self, guide: dict, user_goal: str, user_confirmed_tables: str, user_confirmed_connection: str, knowledge_phase_completed: str) -> dict:
    ...
    def _handle_create_models_yaml_action(self, guide: dict, user_goal: str, user_confirmed_tables: str, user_confirmed_connection: str, knowledge_phase_completed: str) -> dict:
    ...
    def _handle_create_entity_vars_action(self, guide: dict, user_goal: str, user_confirmed_tables: str, user_confirmed_connection: str, knowledge_phase_completed: str) -> dict:
    ...
    def _handle_add_date_filtering_action(self, guide: dict, user_goal: str, user_confirmed_tables: str, user_confirmed_connection: str, knowledge_phase_completed: str) -> dict:
    ...
    def _handle_run_pilot_test_action(self, guide: dict, user_goal: str, user_confirmed_tables: str, user_confirmed_connection: str, knowledge_phase_completed: str) -> dict:
    ...
    def _handle_create_propensity_model_action(self, guide: dict, user_goal: str, user_confirmed_tables: str, user_confirmed_connection: str, knowledge_phase_completed: str) -> dict:
    ...
    def _handle_analyze_existing_project_action(self, guide: dict, user_goal: str, user_confirmed_tables: str, user_confirmed_connection: str, knowledge_phase_completed: str) -> dict:
        """Handle the 'analyze_existing_project' action."""
        # Check if basic profiles knowledge is acquired (simplified requirement)
        completed_knowledge = [k.strip().lower() for k in knowledge_phase_completed.split(",") if k.strip()]
        required_knowledge = ["profiles"]  # Only require basic profiles understanding
        missing_knowledge = [k for k in required_knowledge if k not in completed_knowledge]

        if missing_knowledge:
            guide["validation_status"] = "BLOCKED"
            guide["blocked_reasons"].append("❌ BASIC KNOWLEDGE REQUIRED: Must understand profiles concepts to interpret analysis results")
            guide["critical_warnings"].append("🚨 You MUST call about_profiles() first to understand project structure concepts")
            guide["critical_warnings"].append("🚨 This knowledge is essential for explaining analysis results to users")
            guide["next_tools"] = ["about_profiles"]
            guide["workflow_steps"] = [
                "1. MANDATORY: Call about_profiles(topic='profiles') to understand project structure and concepts",
    ...
    def _handle_unknown_action(self, guide: dict, current_action: str) -> dict:
        ...
        supported_actions = [
            "start", "knowledge_gathering", "discover_resources",
            "create_inputs_yaml", "create_models_yaml", "create_entity_vars",
            "add_date_filtering", "run_pilot_test", "create_propensity_model",
    ...
    def _validate_knowledge_phase(self, current_action: str, knowledge_phase_completed: str) -> dict:
        """Validate that required knowledge phase is completed for configuration actions."""
        config_actions = ["create_inputs_yaml", "create_models_yaml", "create_entity_vars"]
        ...
        missing_knowledge = [k for k in required_for_action if k not in completed_knowledge]
        ...
        if missing_knowledge:
            ...
            return {
                ...
                "blocked_reasons": [f"❌ KNOWLEDGE PHASE INCOMPLETE: Missing {missing_knowledge}"],
                "critical_warnings": [
                ...
                "workflow_steps": [
                    f"1. MANDATORY: Call {tool_call} to understand concepts" for tool_call in next_tools_calls
                ] + [
    ...
    def _validate_user_confirmations(self, current_action: str, user_confirmed_tables: str, user_confirmed_connection: str) -> dict:
        ...
        if not user_confirmed_tables.strip():
            return {
                "valid": False,
                "validation_status": "BLOCKED",
                "blocked_reasons": ["❌ MISSING: user_confirmed_tables parameter required"],
                "critical_warnings": [
        ...
        if not user_confirmed_connection.strip():
            return {
                "valid": False,
                "validation_status": "BLOCKED",
                "blocked_reasons": ["❌ MISSING: user_confirmed_connection parameter required"],
                "critical_warnings": [
    ...
    def validate_propensity_model_config(self, project_path: str, model_name: str, snowflake_client) -> dict:
        """
    ...
    def fetch_snowflake_credentials(self, connection_name: str) -> dict:
        """
        Securely fetch Snowflake connection credentials from siteconfig.yaml
        ...
        try:
            ...
            connection_details = {
                "target": target,
                "type": existing_connection["outputs"][target].get("type", "snowflake"),
                "account": existing_connection["outputs"][target]["account"],
                "warehouse": existing_connection["outputs"][target].get("warehouse"),
                "database": existing_connection["outputs"][target].get("dbname"),
                "schema": existing_connection["outputs"][target].get("schema"),
                "role": existing_connection["outputs"][target].get("role"),
                "user": existing_connection["outputs"][target]["user"],
                "password": existing_connection["outputs"][target].get("password"),
                "private_key": existing_connection["outputs"][target].get("privateKey"),
                "private_key_file": existing_connection["outputs"][target].get("privateKeyFile"),
                "private_key_passphrase": existing_connection["outputs"][target].get("privateKeyPassphrase")
            }
            if connection_details["type"] != "snowflake":
                return {
                    "status": "error",
                    "message": f"Connection '{connection_name}' is not a Snowflake connection. Currently, only snowflake connections are supported."
                }
            ...
            logger.error(f"Error fetching Snowflake credentials: {e}")
            return {
                "status": "error",
                "message": f"Error fetching Snowflake credentials: {str(e)}"
            }
...
class PropensityValidator:
    ...
    def _initialize_configs(self) -> None:
        ...
        self.propensity_model = utils.find_model(self.configs['models'], self.model_name, "propensity")
    ...
    def _validate_model_inputs(self, input_tables_map: dict, entity_vars_map: dict) -> bool:
    ...
    def _validate_single_feature(self, input_feature: str, input_tables_map: dict,
                                entity_vars_map: dict) -> None:
    ...
    def _validate_occurred_at_col(self, input_table_config: dict, feature_name: str = None, is_fallback: bool = False) -> None:
        """
    ...
    def _validate_historic_data(self, input_table_config: dict, feature_name: str = None, is_fallback: bool = False) -> None:
        """
        ...
        if table_name in self.result["table_stats"]:
            logger.debug(f"Skipping historic data validation for table: {table_name} because it already has stats")
            return

        if not occurred_at_col:
            logger.debug(f"Skipping historic data validation for table: {table_name} because it doesn't have an occurred_at_col")
            return
        ...
        try:
            ...
            stats_result = self.snowflake_client.raw_query(query, response_type="pandas")
            logger.debug(f"Stats result: {db_table_name}, {stats_result}")

            if not stats_result.empty:
                self._process_table_stats(stats_result, input_table_config, feature_name, is_fallback)


        except Exception as e:
            logger.warning(f"Could not validate historic data for table {db_table_name}: {e}")
            if is_fallback:
                self.result["warnings"].append({
    ...
    def _process_table_stats(self, stats_result, input_table_config: dict, feature_name: str = None, is_fallback: bool = False) -> None:
        """
        ...
        self.result["table_stats"][table_name] = {
            ...
            "occurred_at_col": input_table_config.get("app_defaults", {}).get("occurred_at_col")
        }

        min_required_days = self.propensity_model["model_spec"]["training"]["predict_window_days"]
    ...
    def _validate_all_input_tables_fallback(self, input_tables_map: dict) -> None:
        ...
        for _, input_table_config in input_tables_map.items():
            self._validate_occurred_at_col(input_table_config, feature_name=None, is_fallback=True)
            self._validate_historic_data(input_table_config, feature_name=None, is_fallback=True)
    ...
    def _has_complex_dependencies(self, entity_vars_map: dict) -> bool:
        ...
        for input_feature in model_inputs:
            ...
            if not table_ref or table_ref.get("source_type") != "inputs":
                logger.debug(f"Feature {input_feature} has a non-inputs table reference")
                return True
...
class ProfilesUtils:
    ...
    def load_inputs_config(self, project_path: str, project_config: dict) -> dict:
        ...
        if not combined_inputs["inputs"]:
            raise FileNotFoundError(f"No inputs configuration found in any YAML files in {models_dir}")
                    ...
                    with open(file_path, "r") as file:
                        ...
                        if config:
                            ...
                            if "var_groups" in config:
                                combined_config["var_groups"].extend(config["var_groups"])
                except Exception as e:
                    logger.warning(f"Could not parse {filename}: {e}")

        if not combined_config["models"] and not combined_config["var_groups"]:
            raise FileNotFoundError(f"No models or var_groups configuration found in any YAML files in {models_dir}")
